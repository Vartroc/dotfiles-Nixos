...
do_swap=$(yesno "Decide weather you want a swap partition or not, default size = 16G")
...
echo "Creating /games"
sudo zfs create -o mountpoint=legacy -encryption=off zroot/games
sudo mount --mkdir -t zfs zroot/games /mnt/games
...
while true; do
    read -rp "Which host to install? (Gartroc) " host
    case $host in
        Gartroc ) break;;
...
nix-shell -p git nixFlakes --command \
    "sudo nixos-install --no-root-password --flake \"github:Vartroc/dotfiles-Nixos/${git_rev:-master}#$host\""
